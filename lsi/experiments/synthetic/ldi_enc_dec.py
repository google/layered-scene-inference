...
from data.flowers import data as flowers_data
...
flags.DEFINE_boolean(
    'debug_synth_texture', False, 'Use gt LDI disps instead of predicted')
...
class Trainer(train_utils.Trainer):
  ...
  def define_summary_ops(self):
    ...
    for p in range(opts.n_layers):
      ...
      if opts.pred_ldi_masks:
        ...
        tf.summary.image(
            'trg_mask_pred_layer_%d' % p,
            tf.cast(self.ldi_trg[1][p]*255, tf.uint8))

    for p in range(opts.n_layers):
      tf.summary.histogram(
          'pred_disp_' + str(p), self.ldi_src[2][p])
  ...
  def feed(self):
    ...
    if opts.dataset == 'synthetic' and opts.synth_dl_eval_data:
      img_src, img_trg, k_s, k_t, rot_mat, trans_mat, _, _, disp_s_fg, disp_s_bg, disp_t_fg, disp_t_bg, _, _ = data_batch
  ...
  def define_loss_graph(self):
    ...
    if opts.l0_self_cons:
      ...
      self.self_cons_loss_src = loss.zbuffer_composition_loss(
          self.ldi_src[0], self.ldi_src[1], self.ldi_src[2],
          self.imgs_src,
    ...
    if opts.l0_self_cons:
      ...
      self.self_cons_loss_trg = loss.zbuffer_composition_loss(
          self.ldi_trg[0], self.ldi_trg[1], self.ldi_trg[2],
          self.imgs_trg,
          zbuf_scale=opts.zbuf_scale,
          bg_layer_disp=opts.bg_layer_disp, max_disp=opts.max_disp)
    ...
    for sp_loss_type in ['indep', 'compose']:
      ...
      for loss_img_name in ['trg', 'src']:
        ...
        tf.summary.image(
            ...
            tf.cast(tf.argmin(  # min across layers
                lwise_splat_loss, axis=0), tf.float32))
        if lwise_splat_loss.get_shape().as_list()[0] == 2:
          ...
          tf.summary.image(
              loss_img_name + '_splat_loss_diff',
              loss_diff)

    ## Smooth Disp Loss
    self.disp_smoothness_loss = 0
    self.disp_smoothness_loss += ldi_utils.disp_smoothness_loss(
        self.ldi_src[2])
    self.disp_smoothness_loss += ldi_utils.disp_smoothness_loss(
        self.ldi_trg[2])
    ...
    self.incr_depth_loss += loss.decreasing_disp_loss(
        self.ldi_src[2])
    self.incr_depth_loss += loss.decreasing_disp_loss(
        self.ldi_trg[2])
