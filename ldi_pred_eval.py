...
flags.DEFINE_boolean(
    'batch_norm_training', True, 'Use training mode for batch norm')
...
flags.DEFINE_boolean(
    'kitti_dl_disparities', True, 'Output disparities'
)
...
class Tester(test_utils.Tester):
  ...
  def define_pred_graph(self):
    ...
    if opts.dataset == 'synthetic':
      ...
      self.src_gt_disp = tf.placeholder(
          tf.float32, [bs, img_height, img_width, 1],
          name='src_gt_disp')
      ...
      self.src_gt_tex_bg = tf.placeholder(
          tf.float32, [bs, img_height, img_width, 3],
          name='src_gt_disp_bg')
    ...
    self.ldi_src = nets.ldi_predictor(
        feat_dec_src, n_layers=n_layers,
        n_layerwise_steps=opts.n_layerwise_steps, skip_feat=skip_feat_src,
        pred_masks=opts.pred_ldi_masks,
        is_training=opts.batch_norm_training
    )
    self.ldi_src[2] *= opts.max_disp
    self.ldi_trg = nets.ldi_predictor(
        feat_dec_trg, n_layers=n_layers, reuse=True,
        n_layerwise_steps=opts.n_layerwise_steps, skip_feat=skip_feat_trg,
        pred_masks=opts.pred_ldi_masks,
        is_training=opts.batch_norm_training
    )
  ...
  def feed(self):
    ...
    if opts.dataset == 'synthetic' and opts.synth_dl_eval_data:
      img_src, img_trg, k_s, k_t, rot_mat, trans_mat, _, _, disp_s_fg, disp_s_bg, disp_t_fg, disp_t_bg, img_s_bg, img_t_bg = data_batch
    elif opts.dataset == 'kitti' and opts.kitti_dl_disparities:
      img_src, img_trg, k_s, k_t, rot_mat, trans_mat, disp_s_fg, disp_t_fg = data_batch
    else:
    ...
    if (opts.dataset == 'synthetic' and opts.synth_dl_eval_data) or (
  ...
  def define_visuals(self):
    ...
    for l in range(self.opts.n_layers):
      ...
      self.visuals['src_disp_pred_layer_%d' % l] = (
          self.ldi_src[2][l][b, :, :, 0])/self.opts.max_disp
  ...
  def define_metrics(self):
    ...
    for loss_img_name in ['trg', 'src']:
      if loss_img_name == 'trg':
        ...
        recons_splat, _, recons_disp = ldi_utils.forward_splat(
            self.ldi_src, self.pixel_coords,
            self.k_s, self.k_t, self.rot_mat, self.trans_mat,
            ...
            bg_layer_disp=opts.bg_layer_disp, max_disp=opts.max_disp)
        if opts.dataset == 'synthetic':
          valid_mask = tf.cast(
              tf.greater(self.trg_gt_disp, opts.bg_layer_disp), tf.float32)
        else:
          valid_mask = tf.ones(
              (opts.batch_size, opts.img_height, opts.img_width, 1))
        ...
        recons_splat, _, recons_disp = ldi_utils.forward_splat(
            self.ldi_trg, self.pixel_coords,
            self.k_t, self.k_s, self.inv_rot_mat, self.inv_trans_mat,
            focal_disps=self.focal_disps,
            compose_layers=True,
            compute_trg_disp=True,
            trg_downsampling=opts.trg_splat_downsampling,
        ...
        if opts.dataset == 'synthetic':
          ...
          valid_mask = tf.ones((opts.batch_size, opts.img_height, opts.img_width, 1))

      ## Forward splatting loss
      to_recons_img_downsampled = tf.image.resize_images(
          to_recons_img, recons_splat.get_shape().as_list()[2:4],
          method=tf.image.ResizeMethod.AREA)
      valid_mask = tf.image.resize_images(
          valid_mask, recons_splat.get_shape().as_list()[2:4],
          method=tf.image.ResizeMethod.AREA)
      # ignore some pixels which might have aliasing
      ...
      if compute_disoccluded_error:
        ...
        to_recons_disocc_mask_ds = tf.image.resize_images(
            to_recons_disocc_mask, recons_splat.get_shape().as_list()[2:4],
            method=tf.image.ResizeMethod.AREA)
      ...
      if compute_depth_error:
        to_recons_disp_ds = tf.image.resize_images(
            to_recons_disp, recons_splat.get_shape().as_list()[2:4],
            method=tf.image.ResizeMethod.AREA)
        pwise_depth_splat_loss = tf.reduce_min(  # min across layers
            tf.reduce_mean(  # mean across channels
                tf.abs(to_recons_disp_ds - recons_disp), axis=4)
            , axis=0)

      pwise_splat_loss = tf.reduce_min(  # min across layers
          tf.reduce_mean(  # mean across channels
              tf.abs(to_recons_img_downsampled - recons_splat), axis=4)
          , axis=0)
      ...
      if compute_disoccluded_error:
        self.visuals[
            '{}_disocc_mask'.format(loss_img_name)
        ] = to_recons_disocc_mask_ds[0]*centre_mask[0]
      ...
      self.visuals[
          '{}_vs_loss'.format(loss_img_name)] = pwise_splat_loss[0]
      self.visuals[
          '{}_splat_tex'.format(loss_img_name)] = recons_splat[0, 0]

      if compute_depth_error:
        self.visuals[
            '{}_gt_disp'.format(loss_img_name)] = to_recons_disp[0, :, :, 0]
        self.visuals[
            '{}_splat_disp'.format(loss_img_name)] = recons_disp[0, 0, :, :, 0]
        self.visuals[
            '{}_depth_loss'.format(loss_img_name)] = pwise_depth_splat_loss[0]

    self.metrics = {
        'compose_loss': self.compose_splat_loss
    }

    self.metrics_norm = {
        'compose_loss': self.valid_pixel_count
    }
    ...
    if compute_bg_pred_error:
      ...
      src_bg_tex_error = tf.abs(
          self.ldi_src[0][opts.n_layers - 1] - self.src_gt_tex_bg)
      trg_bg_tex_error = tf.abs(
          self.ldi_trg[0][opts.n_layers - 1] - self.trg_gt_tex_bg)
      ...
      self.valid_bg_pixel_count += tf.reduce_sum(
          valid_mask_trg + valid_mask_src)

      src_bg_disp_error = tf.abs(
          self.ldi_src[2][opts.n_layers - 1] - self.src_gt_disp_bg)
      trg_bg_disp_error = tf.abs(
          self.ldi_trg[2][opts.n_layers - 1] - self.trg_gt_disp_bg)
    ...
    if compute_fg_pred_error:
      ...
      self.valid_fg_pixel_count += tf.reduce_sum(
          valid_mask_trg + valid_mask_src)
